...
During our fifth semester, we found ourselves constantly switching between Gmail, Google Calendar, GitHub, and various productivity tools while managing our coursework and projects. This fragmentation frustrated us—we'd spend minutes just navigating between apps to complete simple tasks like "check my emails and add important deadlines to my calendar." We wondered: why can't we just tell an AI assistant to do this?

We explored existing solutions like ChatGPT and Google Assistant, but they had critical limitations. ChatGPT couldn't access our Gmail or Calendar. Google Assistant worked only in English and lacked deep integration with developer tools like GitHub. More importantly, both required sending our data to cloud servers, raising privacy concerns. We needed something we could host ourselves, customize for our workflow, and use in our native languages.

This led us to build Kuma, an AI-powered personal assistant that we could actually use daily. Our goal was simple: create a system where we could have natural conversations in Hindi or English, ask it to manage our emails and calendar, analyze documents through our phone camera, and integrate with the tools we use every day—all while keeping our data on our own server.

We built Kuma using TypeScript with Bun 1.0 runtime for the backend (we chose Bun over Node.js after benchmarking showed 3x faster startup times) and React 18 with Vite for the frontend. For AI capabilities, we integrated Google Gemini 1.5 Flash and OpenAI GPT-4o through Vercel AI SDK, which gave us streaming responses that appear word-by-word like ChatGPT. We implemented a multi-agent architecture where a router agent analyzes each query and delegates it to specialized agents—one for research, one for stock market analysis, and one for financial tasks. This approach reduced hallucinations compared to using a single general-purpose agent.

The most challenging part was adding voice interaction with Indic language support. We integrated Sarvam AI for Hindi, Tamil, and Telugu speech recognition and synthesis, combined with LiveKit for real-time audio streaming. We also added vision capabilities using Google Gemini Vision, allowing users to upload images or documents for OCR and analysis. To handle long-running AI operations without timeout errors, we implemented asynchronous processing using Redis 7.2 Streams with a worker process pattern. Finally, we containerized everything with Docker to ensure our project runs consistently across different environments.
